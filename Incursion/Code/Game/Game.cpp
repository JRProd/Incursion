...
void Game::DebugRender() const
{
    ...
    if ( g_InputSystem->GetXboxController( 0 ).IsConnected() )
    {
        ...
        Disc leftDisc = Disc( Vec2( 15.f, 15.f ), circleRadius );
        Disc rightDisc = Disc( Vec2( MAX_UI_WIDTH - 15.f, 15.f ), circleRadius );

        Disc leftUnit = Disc::UNIT_DISC;
        Disc rightUnit = leftUnit.GetTranslation( rightDisc.center );
        leftUnit.Translate( leftDisc.center );

        g_Renderer->DrawDisc( leftDisc, Rgba8::GREEN, .2f );
        g_Renderer->DrawDisc( leftDisc.GetScaledRadiusLength(
            gamepad.GetLeftJoystick().m_InnerDeadzoneFraction ),
            Rgba8::GREEN, .1f );
        g_Renderer->DrawDisc( leftDisc.GetScaledRadiusLength(
            gamepad.GetLeftJoystick().m_OuterDeadzoneFraction ),
            Rgba8::GREEN, .1f );
        g_Renderer->DrawDisc( rightDisc, Rgba8::GREEN, .2f );
        g_Renderer->DrawDisc( rightDisc.GetScaledRadiusLength(
            gamepad.GetRightJoystick().m_InnerDeadzoneFraction ),
            Rgba8::GREEN, .1f );
        g_Renderer->DrawDisc( rightDisc.GetScaledRadiusLength(
            gamepad.GetRightJoystick().m_OuterDeadzoneFraction ),
            Rgba8::GREEN, .1f );

        // Draw Left Joystick Debug
        g_Renderer->DrawDisc( leftUnit.GetTranslation( gamepad.GetLeftJoystick().GetPosition() * circleRadius ),
                              Rgba8::GREEN,
                              .1f );
        g_Renderer->DrawDisc( leftUnit.GetTranslation( gamepad.GetLeftJoystick().GetRawPosition() * circleRadius ),
                              Rgba8::RED,
                              .1f );
        g_Renderer->DrawLine( leftDisc.center,
                              leftDisc.center + gamepad.GetLeftJoystick().GetPosition() * circleRadius,
                              Rgba8::GREEN,
                              .1f );
        g_Renderer->DrawLine( leftDisc.center,
                              leftDisc.center + gamepad.GetLeftJoystick().GetRawPosition() * circleRadius,
                              Rgba8::RED,
                              .1f );

        // Draw Right Joystick Debug
        g_Renderer->DrawDisc( rightUnit.GetTranslation( gamepad.GetRightJoystick().GetPosition() * circleRadius ),
                              Rgba8::GREEN,
                              .1f );
        g_Renderer->DrawDisc( rightUnit.GetTranslation( gamepad.GetRightJoystick().GetRawPosition() * circleRadius ),
                              Rgba8::RED,
                              .1f );
        g_Renderer->DrawLine( rightDisc.center,
                              rightDisc.center + gamepad.GetRightJoystick().GetPosition() * circleRadius,
                              Rgba8::GREEN,
                              .1f );
        g_Renderer->DrawLine( rightDisc.center,
                              rightDisc.center + gamepad.GetRightJoystick().GetRawPosition() * circleRadius,
                              Rgba8::RED,
                              .1f );

        Vec2 leftTriggerStart = leftDisc.center + Vec2( circleRadius + 5.f, -circleRadius );
        Vec2 rightTriggerStart = rightDisc.center + Vec2( -circleRadius - 5.f, -circleRadius );
        float triggerDistance = circleRadius * 2.f;
        // Draw Left Trigger Debug
        g_Renderer->DrawLine( leftTriggerStart,
                              leftTriggerStart + Vec2( 0.f, 1.f ) * triggerDistance * gamepad.GetLeftTrigger(),
                              Rgba8::CYAN,
                              .5f );
        g_Renderer->DrawLine( rightTriggerStart,
